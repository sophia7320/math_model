...
> **2026-09-13 v1.4**：执行层为逐槽因果执行；Q2–Q4 采用两日滚动、日末 SOC 不固定并跨日传递。
> Q3 消融后正式关闭场景对冲，对冲仅保留为负结果扩展。
...
| 问题二 自适应加权（口径 E，**官方 result2**） | 完成 | **EWMA h=5 + κ=1.02 + m=25 kW = 1395.7 万元**（计划 1316.0 + 紧急 79.6）；纯历史、不用附件3预报 | `results/result2.xlsx`、`figures/Q2E_*.pdf`、`code/outputs/q2e_tune_*.csv` |
| 问题二 ARIMA 预测对照 | 完成 | ARIMA(2,0,1) **2464.6 万元**（负荷 MAE 638.9、光伏 152.6 kW；未建模周周期） | `figures/Q2ARIMA_*.pdf`、`code/outputs/q2_arima_*.csv` |
| 问题三 日内滚动调整 | 完成 | 正式方案（组合 λ=0.7 + 三点调整、无对冲）**1347.1 万元**；相对无调整官方 −8.01%、相对官方三点 −0.76%；对冲扩展 1349.2 万 | `results/result3.xlsx`、`figures/Q3_*.pdf`、`reports/Q3_对冲取舍实验.md` |
| 问题四 波动电价 | 完成 | G 为题面主口径：Q2 层 1459.8 万、Q3 层 1410.3 万；H 扩展 1465.9 / 1418.4 万 | `results/result4-2.xlsx`、`result4-3.xlsx`、`figures/Q4_*.pdf` |
| 论文 | 完成 | LaTeX（cumcmthesis 模板，19 页）；v1.4 验收 PASS | `paper/main.pdf`、`论文_CUMCM2026_C题.pdf`、`reports/VERIFY_REPORT.md` |

口径说明：`result2.xlsx`（官方结果文件）由**口径 E 调优版（EWMA h=5 + κ=1.02 + m=25 kW）**生成
（1395.7 万元；旧 W=7 版备份在 `code/outputs/result2_W7_backup.xlsx`，旧口径 D 版备份在
...
uv run python -m solve.q3           # 问题三正式无对冲方案：result3 + 3 张图 + 报告章节
