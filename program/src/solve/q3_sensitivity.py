"""Q3 对冲扩展灵敏度：情景数 / 场景池窗口 / 组合权重 λ（不进入正式结果）。

对照 GPT 版"场景数 5/10/20 + λ 窗口"的灵敏度思路；本工作区 Q3 的 λ 为固定值
（无滚动窗口），因此等价地补测：
  - 对冲情景数 n_scen ∈ {5, 10, 20, 40}，并对 10/20/40 使用 3 个随机种子
  - 场景池回看窗口 lookback ∈ {30, 60, 90}（扩展基准 90）
  - 组合权重 λ ∈ {0.5, 0.7, 0.9}（扩展基准 0.7）
...
VARIANTS = [
    ("对冲扩展基准（情景 40 / seed=7）", dict(n_scen=40, pmm=14, plb=90, lam=0.7, seed=7)),
...
def main():
    ...
    out["相对扩展基准/%"] = (100 * (out["总费用/万元"] / base - 1)).round(2)
    out["场景前视违规天数"] = viol
