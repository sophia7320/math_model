...
口径（论文假设，常量区可调）：
- 计划：每天 0:00 做 48 小时滚动计划（core.lp.plan_horizon，视野末端完全自由），
  只执行当日；计划购电量 take-or-pay，储能日末 SOC 自由并跨日传递；
- 执行：逐槽因果执行（core.causal.exec_segment_causal，统一 free 策略，
...
本模块保留既有入口与公开 API（兼容门面）：
    ...
    uv run python -m solve.q2 --write-result2-d   # 口径 D 对照写 result2_D_backup.xlsx
...
from solve.core.lp import exec_day, plan_day, plan_horizon
...
from solve.models.errors import scenario_from_residual as _scenario_from_residual
...
def run_q2(n_mc: int = 100, seed: int = 42, hedge: bool = True,
    ...
    pm.record_result(
        ...
        note=(
            "口径 D：0:00 用附件 3 的 0:00 光伏预报制定计划（负荷视为已知），"
            "48 小时滚动视野、只执行首日，储能日末 SOC 自由并跨日传递；"
            "计划购电 take-or-pay，实际按附件 2 执行，缺口按 5 倍交易时刻电价紧急购电；"
            "结果自 2 月 1 日报送；图中位置：figures/Q2_逐日紧急购电.pdf。"
        ),
    )
