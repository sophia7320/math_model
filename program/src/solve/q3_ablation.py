...
主方案 = 组合（λ=0.7，平滑 β=0.1）+ 三点调整 + 6/12 无前视对冲 → 1327.8 万（RESULTS_REPORT）。
本脚本复算主方案并补充三个消融配置：
...
def main():
    ...
    rows = [
        ...
        run_cfg("主方案（组合+平滑+对冲）", qp.simulate_day_rt_hedge,
    ...
    record(
        ...
        df,
        note=(
            "因果执行口径下的单项消融：官方·三点+对冲（无组合与平滑）；组合·三点·无对冲；"
            "主方案（组合 λ=0.7 + 平滑 β=0.1 + 三点 + 6/12 对冲）；"
            "信息退化（计划组合、调整层仅官方）。用于论文中“对冲/组合/同口径”的贡献分解。"
            "对照：无调整官方 1521.4 万、三点官方 1422.9 万（RESULTS_REPORT 主方案章节）。"
        ),
    )
