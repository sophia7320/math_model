...
结论（2026-09-13）：free 显著优于 target（夏季 −40.3 万/12.2%，秋季 −8.4 万/4.3%），
dayend 与 free 同值；SOC 仍全幅利用（[1200, 10800]）。
...
def main() -> None:
    ...
    record(
        "执行器段末目标机制对照（小样本）",
        df,
        note=("统一结构下对比 target（段末硬目标）/ dayend（仅日末）/ free（无目标）；"
              "free 为现行统一口径（consistency.EXEC_POLICY）；"
