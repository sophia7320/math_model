...
KAPPA = 1.015            # 负荷预测抬升系数（时间留出选定）
MARGIN = 75.0            # 光伏预测折减（kW）
...
EXEC_POLICY = "free"      # 执行器策略：无段末硬目标，仅容量/功率约束
#   依据（reports/执行器段末目标实验.md）：段末目标会迫使"紧急购电充能/扣留放电"，
#   60 天夏季样本 +40.3 万（12.2%）、47 天秋季样本 +8.4 万（4.3%）；
#   "free" 与 "dayend" 在样本内同值，取更简洁的 free。
