"""项目级模型一致性规范：**唯一参数源**与统一构造（2026-09-13，v1.4）。
...
3. 正式 Q3/Q4-3 不使用场景对冲（``Q3_USE_HEDGE=False``）；联合残差情景仅保留为
   可复现实验扩展，池严格取目标日之前。
...
7. Q3 决策：正式组合权重由 ``Q3_LAM``/``Q3_ADJ_LAM`` 给出，是否对冲只由
   ``Q3_USE_HEDGE`` 控制；v1.4 固定为关闭。
8. Q1 豁免：题面明确单日且要求 0:00 = 24:00，保持日循环；仅共用物理常数
   （见 ``solve.common``）。
"""
...
KAPPA = 1.02             # 负荷预测抬升系数（全局定稿：Q2 新表留出冠军，Q2+Q3 交叉核对）
MARGIN = 25.0            # 光伏预测折减（kW）（同上）
N_SCEN = 40              # 对冲/评估等概率情景数
SCEN_MIN_SAME_MONTH = 14  # 情景池同月最少天数
SCEN_LOOKBACK = 90        # 同月不足时的回看窗口（天）
Q3_LAM = 0.7             # Q3 0:00 组合权重（官方预报占比）
Q3_ADJ_LAM = 0.7         # Q3 调整层组合权重（保持信息口径一致）
Q3_USE_HEDGE = False     # v1.4 正式决策：消融显示对冲增费，故正式方案关闭
