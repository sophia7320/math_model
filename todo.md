...
⚠ **2026-09-13 新发现并修复**：`simulate_day_rt_hedge` 末段曾绕过 free 开关（旧 free 实为 dayend）；
修复后小样本 free 相对 target −13.6%/−4.5%——**Q3 的 1360.3 万为修复前混合口径，须重算**；
...
- [ ] **P0-2 Q3 参数搜索在修正后的执行器下重跑**
  ```powershell
  uv run python -m solve.q3_tune     # ~20 分钟（16+5 组，6 进程）
  ```
  ⚠ 前提已完成：末段漏洞修复（free 现在才是真自由）；旧的 1360.3/1357.1 对照为修复前数字，
  重跑后一并更新。重跑后若最优参数/策略变化（例如 κ/m 收向 1.02/50、λ 更低或"λ=1+关对冲"），
  更新 `q3.py` 的 `LAM/ADJ_LAM/hedge` 并 `uv run python -m solve.q3` 重算 result3（~5 分钟）；
  同时决定是否将 `consistency.py` 的 κ/m 定稿为 Q2 重选值。
...
| `results/result3.xlsx` | 1360.3 万（旧：计划 1298.2 + 调整 +20.6 + 紧急 41.6；参数 1.015/75） | **修复前混合口径（= dayend）产物，待重算** |
| `results/result4-2.xlsx` | G 口径过渡版本 | 待 P1-1 重算 |
| `results/result4-3.xlsx` | G 口径过渡版本 | 待 P1-1 重算 |

对照（**修复前**数字，待重算）：Q3 无调整 1460.8 → 三点官方 1357.1 → 主方案 1360.3；
末段修复后 free 还会有少量下降（小样本 −1.6%/−0.3% 量级），P0-2 重选参后一并定稿。
