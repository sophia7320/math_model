...
统一口径（EWMA h=5 + κ=1.015 + m=75 + （Δ负荷, Δ光伏）联合残差 40 情景 + 因果负荷 +
2 日滚动/末端自由 + **执行器无段末硬目标**）已全部落地；
**Q3 已重算 = 1360.3 万**（旧"Q3 > Q2"异常已消除）；
**Q2 官方（1406.4 万）仍是旧执行器产物，待重算**；Q4 两层为过渡版本；论文数值待统一同步。
...
- [ ] **P0-1 Q2 官方按自由执行器重算**
  ```powershell
  cd program; $env:PYTHONIOENCODING='utf-8'
  uv run python -m solve.q2_tune --rolling-holdout   # ~3 分钟：κ/m 留出重选（执行器变更后需复核）
  # 若冠军仍是 κ=1.015/m=75 → 直接：
  uv run python -m solve.q2_tune --result2           # ~1-2 分钟：重写 result2 + Q2 三图 + 官方 MC
  ```
  注意：若留出冠军变化，先更新 `q2_tune.write_result2_tuned` 默认参数再写 result2。
...
## 五、当前正式数字速查（2026-09-13 05:55）

| 文件 | 数值 | 状态 |
| --- | --- | --- |
| `results/result1.xlsx` | 35126.95 元/日（59482.7 kWh，弃光 0） | 正式（题面日循环） |
| `results/result2.xlsx` | 1406.4 万（计划 1321.9 + 紧急 84.5） | **旧执行器产物，待 P0-1 重算** |
| `results/result3.xlsx` | **1360.3 万**（计划 1298.2 + 调整 +20.6 + 紧急 41.6） | 已按 v1.1 重算 ✓ |
